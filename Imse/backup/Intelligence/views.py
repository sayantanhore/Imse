...
# Refer to Path.py for current dataset
...
def firstround_search(request):
	...
	images_to_show = initial_image_selector.load_images(no_of_images)
	# Set images to session
	request.session['images_shown'] = images_to_show
...
def do_search(request, state = 'nostart'):
    ...
    if state == 'start':
        ...
        images_shown = request.session['images_shown']
...
def do_search(request, state='notstart'):
