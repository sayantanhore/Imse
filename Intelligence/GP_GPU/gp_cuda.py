...
def gaussian_process(data, feedback, feedback_indices, float_type=np.float32, int_type=np.int32,
                     ...
                     K_xx_noise=None):
    print("Enter >> 'gaussian_process'")
    #t = time.time()
    if kernel_file == None:
        kernel_file = os.path.dirname(os.path.realpath(__file__)) + '/kernels.cu'
    if debug:
...
#            feedback_indices = np.loadtxt(infile)
    ...
    print("Cuda module type :: " + str(type(cuda_module)))
    try:
        cuda_module = SourceModule(cuda_module)
    except Exception as e:
        raise Exception(e)
    logger.debug(type(cuda_module))
    # Inialize variables
    ...
    if debug:
        ...
        K_test = dist.cdist(K_test_features, K_test_features, DISTANCE_METRIC) / n_features + np.diag(K_noise)
        drv.memcpy_dtoh(K, K_gpu)
    ...
    if debug:
        ...
        test_K = dist.cdist(test_K_features, test_K_features, DISTANCE_METRIC) / n_features + np.diag(test_K_noise)
        test_K_inv = np.linalg.inv(test_K[:n_feedback, :n_feedback])
        print('Test_K_inv', str(np.allclose(test_K_inv, K_inv[:n_feedback, :n_feedback])))
        ...
        test_variance = np.abs(np.subtract(test_K_xx, test_K_xKK_x_T))
        print('Test_K_inv', str(np.allclose(test_K_xK, K_xK[:n_predict, :n_feedback])))
        test_mean = np.dot(test_K_xK, feedback)
        print('test_mean shape: ' + np.shape(test_mean))
        ...
        print('Mean differences (first 10):', np.subtract(mean.flatten()[:10], test_mean.flatten()[:10]))
