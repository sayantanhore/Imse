...
class LoadInitialImages(object):
	...
	self.colors = colors
        self.b = 8
        self.images_number = settings.IMAGENUM
    ...
    def load_images(self, no_of_images):
        print("Enter >> 'load_images'")
        print(os.getcwd())
        image = Image.new("RGB", (300, 300))
        imdraw = ImageDraw.Draw(image)

        interval = 300 / len(self.colors)
        print("before rectangles")
        for i in range(len(self.colors)):
            imdraw.rectangle([(0, i * interval), (300, (i + 1) * interval)], fill=self.colors[i])
            # image.save("/data/Imse/Data/123.jpg")
        try:
            image.save(self.data_path + "123.jpg")
            subprocess.call(["chmod", "777", os.path.join(self.data_path, '123.jpg')])
        except Exception as e:
            print(e)
        print("Image created")
        image_in_cv = cv.LoadImageM(self.data_path + "123.jpg")
        hist = self.rgb_histogram(image_in_cv)
        hist_rgb = (np.array(hist.bins)).reshape(-1)

        np.savetxt(self.data_path + "cl_dist_temp2.txt", hist_rgb.T)
        subprocess.call(["chmod", "777", os.path.join(self.data_path, 'cl_dist_temp2.txt')])
        print("Hist Extracted")
        centroids = np.load(self.data_path + "cl-centroids-" + str(self.images_number) + ".npy")
        print("Centroid picked")
        clusters_to_datapoints = np.load(self.data_path + "clusters-to-datapoints-cl-" + str(self.images_number))
        print("Cluster picked")
        hist_rgb = hist_rgb[np.newaxis]
        print("Scipy path :: " + inspect.getfile(scipy))
